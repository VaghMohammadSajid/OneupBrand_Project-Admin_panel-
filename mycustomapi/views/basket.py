...
class AddProductView(APIView):
    ...
    def validate(
        ...
        all_categories = product.parent.categories.all()

        categories_list = []
        for single_categories in all_categories:
            print(single_categories.get_ancestors_and_self())
            categories_list.extend(single_categories.get_ancestors_and_self())

        flag = True
        try:
            all_vouchers = basket.vouchers.all()
            if all_vouchers.exists():

                for single_voucher in all_vouchers:
                    all_offer = single_voucher.offers.all()

                    if all_offer.exists():
                        for single_offer in all_offer:
                            try:
                                if single_offer.condition.range.back_range:
                                    range_object = (
                                        single_offer.condition.range.back_range
                                    )
                                    logger.debug(product.__dict__)
                                    if range_object._all_product.filter(
                                        ...
                                        logger.debug(
                                            "reached inside can't add product area"
                                        )
                            ...
                            if single_offer.benefit.range.includes_all_products:
                                flag = False

                                break
                            logger.debug("reached first stage")
                            if all_categories.exists():
                                if not single_offer.benefit.range.contains_product(
                                    product
                                ):
                                    logger.debug(flag)
        ...
        if flag == True:
            my_message = "Product not included in Voucher"
            return False, my_message
