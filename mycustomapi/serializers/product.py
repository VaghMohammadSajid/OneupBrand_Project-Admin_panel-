...
from mycustomapi.utils.exists import bound_unique_together_get_or_create_multiple
from mycustomapi.utils.loading import get_api_classes
from mycustomapi.utils.settings import overridable
from mycustomapi.utils.files import file_hash
from mycustomapi.utils.exists import find_existing_attribute_option_group
from mycustomapi.utils.accessors import getitems

from mycustomapi.serializers.fields import DrillDownHyperlinkedIdentityField
...
class ChildProductSerializer(PublicProductSerializer):
    ...
    specifications = serializers.CharField(source="parent.specifications")
    is_stockrecords = serializers.SerializerMethodField()
    is_num_allocated = serializers.SerializerMethodField()
    ...
    def get_image(self, obj):
        objec = ProductImage.objects.filter(product=obj).values("original")

        return objec

    def get_is_stockrecords(self, obj):
        stock_obj = StockRecord.objects.filter(product=obj).first()
        return stock_obj.num_in_stock

    def get_is_num_allocated(self, obj):
        stock_obj = StockRecord.objects.filter(product=obj).first()

        return stock_obj.num_allocated
...
class ProductSerializer(PublicProductSerializer):
    ...
    # is_all_child = serializers.SerializerMethodField()
    class Meta(PublicProductSerializer.Meta):
        fields = overridable(
    ...
    #     check_obj=WishListLine.objects.filter(product=obj)
    #     if check_obj.exists():

    #         wish_obj = WishListLine.objects.filter(product=obj).first()
    #         return wish_obj.is_wishlist
    #     else:
    ...
    # def get_is_all_child(self,obj):
    #     all_product = Product.objects.filter(id=obj.id).values('parent')
    #     parent_id = all_product[0]['parent']

    #     if parent_id != None:
    #         obj=Product.objects.filter(parent=parent_id)
    #         all_pro=Product.objects.filter(parent=parent_id).values('id','title','parent','parent__title','structure','description','upc','is_discountable','rating','slug','description','meta_title','meta_description','is_discountable','attributes','product_class')
    #         arrayProd = []
    #         for data in all_pro:
    #             dynData = data
    #             jjj = data['id']
    ...
    #             dynData["image"] = objec
    #             attributes=ProductAttributeValue.objects.filter(product=jjj).values('attribute__name','value_text','value_integer','value_multi_option__option','value_boolean','value_float','value_richtext','value_option__option')
    #             dynData["attributes"] = attributes

    #             stock_record = StockRecord.objects.filter(product=jjj).values('mrp', 'discount','discount_type','gst_rate__igst_rate','base_price')
...
class ProductLinkSerializer(ProductSerializer):
    ...
    class Meta(PublicProductSerializer.Meta):
        fields = overridable(
            ...
            default=("url", "id", "upc", "title", "images", "children"),
        )
...
#### custom serializer for lineproduct use for showing order product details
class LineProductSerializer(serializers.ModelSerializer):
    images = ProductImageSerializer(many=True, required=False)
