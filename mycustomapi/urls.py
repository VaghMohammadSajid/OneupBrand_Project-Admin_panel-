...
from mycustomapi.views.tax import AddGSTGroup, AddGSTSetup
from mycustomapi.views.checkout import CheckOrder
from mycustomapi.utils.loading import get_api_classes, get_api_class
from .views.checkout import WalletTotalAmountAPi
...
(VoucherList, SingleOrder) = get_api_classes(
    "views.checkout",
    ["VoucherList", "SingleOrderApi"],
)
from .views.checkout import FreezeStock, UnfreezeStock
...
urlpatterns = [
    ...
    path(
        "orderswithoutlogin/",
        OrderListwithoutlogin.as_view(),
        name="orderswithoutlogin",
    ),
    path("ordersstatus/", Orderstatusdetails.as_view(), name="orders-status"),
    ...
    path(
        ...
        GetPostCodeInformation.as_view(),
        name="get-postalcode-info",
    ),
    path("get-voucher/", VoucherList.as_view()),
    path("get-single_order/<int:number>/", SingleOrder.as_view()),
    path("wallet-amount/", WalletTotalAmountAPi.as_view()),
    path("freeze-stock/", FreezeStock.as_view()),
    path("unfreeze-stock/", UnfreezeStock.as_view()),
]
from .views.checkout import failed_order_view
...
admin_urlpatterns = [
    ...
    path("order-check/", CheckOrder.as_view()),
]
