...
from mycustomapi.basket.operations import (
    request_allows_access_to_basket,
    store_basket_in_session,
    get_basket,
)

from mycustomapi.utils.loading import get_api_class
from mycustomapi.utils.request import get_domain
from mycustomapi.utils.session import session_id_from_parsed_session_uri, get_session
from mycustomapi import models
...
class ApiBasketMiddleWare(BasketMiddleware, IsApiRequest):
    ...
    mycustomapi can not do that, because we don't want to put the burden
    of managing a cookie jar on mycustomapi clients that are not websites.
    """
    ...
    def __call__(self, request):
        if self.is_api_request(request):
            ...
            # session baskets, since mycustomapi uses only baskets from the
            # session.
    ...
    def process_response(self, request, response):
        if (
            ...
            # could have been created by mycustomapi.
            cookie_key = self.get_cookie_key(request)
